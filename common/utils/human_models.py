...
class SMPLX(object):
    def __init__(self):
        ...
        with open(
            osp.join(cfg.human_model_path, "smplx", "SMPLX_to_J14.pkl"), "rb"
        ) as f:
            self.j14_regressor = pickle.load(f, encoding="latin1")
        with open(
            osp.join(cfg.human_model_path, "smplx", "MANO_SMPLX_vertex_ids.pkl"), "rb"
        ) as f:
            self.hand_vertex_idx = pickle.load(f, encoding="latin1")
        self.face_vertex_idx = np.load(
            osp.join(cfg.human_model_path, "smplx", "SMPL-X__FLAME_vertex_ids.npy")
        )
...
class SMPL(object):
    def __init__(self):
        ...
        self.layer = {
            "neutral": smplx.create(
                cfg.human_model_path, "smpl", gender="NEUTRAL", **self.layer_arg
            ),
            "male": smplx.create(
                cfg.human_model_path, "smpl", gender="MALE", **self.layer_arg
            ),
            "female": smplx.create(
                cfg.human_model_path, "smpl", gender="FEMALE", **self.layer_arg
            ),
        }
        ...
        self.orig_root_joint_idx = self.orig_joints_name.index("Pelvis")
        self.orig_joint_regressor = (
            self.layer["neutral"].J_regressor.numpy().astype(np.float32)
        )
