...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.joint_set = {
            ...
            "regressor": np.load(
                osp.join("..", "data", "Human36M", "J_regressor_h36m_smplx.npy")
            ),
        }
    ...
    def load_data(self):
        ...
        for subject in subject_list:
            ...
            with open(
                osp.join(
                    self.annot_path, "Human36M_subject" + str(subject) + "_data.json"
                ),
                "r",
            ) as f:
                annot = json.load(f)
            ...
            with open(
                osp.join(
                    self.annot_path, "Human36M_subject" + str(subject) + "_camera.json"
                ),
                "r",
            ) as f:
            ...
            with open(
                osp.join(
                    self.annot_path,
                    "Human36M_subject" + str(subject) + "_SMPLX_NeuralAnnot.json",
                ),
                "r",
            ) as f:
        ...
        for aid in db.anns.keys():
            ...
            frame_idx = img["frame_idx"]
            cam_idx = img["cam_idx"]
            smplx_param = smplx_params[str(subject)][str(action_idx)][
                str(subaction_idx)
            ][str(frame_idx)]
            ...
            R, t, f, c = (
                np.array(cam_param["R"], dtype=np.float32),
            ...
            joint_world = np.array(
                joints[str(subject)][str(action_idx)][str(subaction_idx)][
                    str(frame_idx)
                ],
                dtype=np.float32,
            )
    ...
    def __getitem__(self, idx):
        ...
        img_path, img_shape, bbox, cam_param = (
            data["img_path"],
            data["img_shape"],
            data["bbox"],
            data["cam_param"],
        ...
        if self.data_split == "train":
            ...
            joint_cam = (
                joint_cam - joint_cam[self.joint_set["root_joint_idx"], None, :]
            ) / 1000  # root-relative. milimeter to meter.
            joint_img = data["joint_img"]
            joint_img = np.concatenate(
                (joint_img[:, :2], joint_cam[:, 2:]), 1
            )  # x, y, depth
            joint_img[:, 2] = (
                (joint_img[:, 2] / (cfg.body_3d_size / 2) + 1)
                / 2.0
            ...
            joint_img, joint_cam, joint_valid, joint_trunc = process_db_coord(
                ...
                smpl_x.joints_name,
            )  # 17 joints num to smplx's 137 joints num
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            action_idx = annot["action_idx"]
            joint_gt = (
                joint_gt - joint_gt[self.joint_set["root_joint_idx"], None]
            )  # root-relative
            joint_gt = joint_gt[self.joint_set["eval_joint"], :]

            # h36m joint from param mesh
            mesh_out = out["smplx_mesh_cam"] * 1000  # meter to milimeter
            joint_out = np.dot(
                self.joint_set["regressor"], mesh_out
            )  # meter to milimeter
            joint_out = (
                joint_out - joint_out[self.joint_set["root_joint_idx"], None]
            )  # root-relative
            joint_out = joint_out[self.joint_set["eval_joint"], :]
            joint_out_aligned = rigid_align(joint_out, joint_gt)
            # eval_result["mpjpe"].append(
            #     np.sqrt(np.sum((joint_out - joint_gt) ** 2, 1)).mean()
            # )
            # eval_result["pa_mpjpe"].append(
            #     np.sqrt(np.sum((joint_out_aligned - joint_gt) ** 2, 1)).mean()
            # )
            mpjpe = np.sqrt(np.sum((joint_out - joint_gt) ** 2, 1)).mean()
            pa_mpjpe = np.sqrt(np.sum((joint_out_aligned - joint_gt) ** 2, 1)).mean()
            eval_result[action_idx]["mpjpe"].append(mpjpe)
            eval_result[action_idx]["pa_mpjpe"].append(pa_mpjpe)
    ...
    def print_eval_result(self, eval_result):
        ...
        for i in range(17):
            # print("Action {}:\t {}".format(i, self.action_name[i]))
            print("Action {}:\t".format(i))
            print("MPJPE: %.2f mm" % np.mean(eval_result[i]["mpjpe"]))
            print("PA MPJPE: %.2f mm" % np.mean(eval_result[i]["pa_mpjpe"]))
