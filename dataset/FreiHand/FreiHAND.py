...
class FreiHAND(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.human_bbox_root_dir = osp.join(
            "..", "data", "FreiHAND", "rootnet_output", "bbox_root_freihand_output.json"
        )
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            if self.data_split == "train":
                cam_param, mano_param = (
                    data[db_idx]["cam_param"],
                    data[db_idx]["mano_param"],
                )
    ...
    def __getitem__(self, idx):
        ...
        img, img2bb_trans, bb2img_trans, rot, do_flip = augmentation(
            img, bbox, self.data_split, enforce_flip=False
        )
        ...
        if self.data_split == "train":
            ...
            (
                ...
                mano_param, cam_param, do_flip, img_shape, img2bb_trans, rot, "mano"
            )
            ...
            inputs = {"img": img}
            targets = {
                "joint_img": np.zeros_like(mano_joint_img),
                "mano_joint_img": mano_joint_img,
                "joint_cam": np.zeros_like(mano_joint_cam),
                "mano_joint_cam": mano_joint_cam,
                "mano_pose": mano_pose,
                "mano_shape": mano_shape,
            }
            meta_info = {
                "joint_valid": np.zeros_like(mano_joint_valid),
                "joint_trunc": np.zeros_like(mano_joint_trunc),
                "mano_joint_trunc": mano_joint_trunc,
                "mano_joint_valid": mano_joint_valid,
