...
class MSCOCO(torch.utils.data.Dataset):
    ...
    def merge_joint(self, joint_img, feet_img, lhand_img, rhand_img, face_img):
        ...
        joint_img = np.concatenate(
            (joint_img, pelvis, lfoot, rfoot, lhand_img, rhand_img, face_img)
        ).astype(
            np.float32
        )  # self.joint_set['joint_num'], 3
        return joint_img
    ...
    def load_data(self):
        if self.data_split == "train":
            ...
            with open(
                osp.join(self.annot_path, "MSCOCO_train_SMPLX_all_NeuralAnnot.json")
            ) as f:
        ...
        if self.data_split == "train":
            ...
            for aid in db.anns.keys():
                ...
                lhand_img = np.array(ann["lefthand_kpts"], dtype=np.float32).reshape(
                    -1, 3
                )
                rhand_img = np.array(ann["righthand_kpts"], dtype=np.float32).reshape(
                    -1, 3
                )
                face_img = np.array(ann["face_kpts"], dtype=np.float32).reshape(-1, 3)
                joint_img = self.merge_joint(
                    joint_img, foot_img, lhand_img, rhand_img, face_img
                )
                ...
                for body_name, part_name in (
                    ("L_Wrist", "L_Wrist_Hand"),
                    ("R_Wrist", "R_Wrist_Hand"),
                    ("Nose", "Face_18"),
                ):
                    if (
                        joint_valid[self.joint_set["joints_name"].index(part_name), 0]
                        == 0
                    ):
                        joint_img[self.joint_set["joints_name"].index(part_name)] = (
                            joint_img[self.joint_set["joints_name"].index(body_name)]
                        )
                        joint_valid[self.joint_set["joints_name"].index(part_name)] = (
                            joint_valid[self.joint_set["joints_name"].index(body_name)]
                        )
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == "train":
            ...
            face_bbox, face_bbox_valid = self.process_hand_face_bbox(
                data["face_bbox"], do_flip, img_shape, img2bb_trans
            )
            if do_flip:
            ...
            face_bbox_center = (face_bbox[0] + face_bbox[1]) / 2.0
            ...
            joint_img = np.concatenate(
                (joint_img[:, :2], np.zeros_like(joint_img[:, :1])), 1
            )  # x, y, dummy depth
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            if annot["lhand_bbox"] is not None:
                ...
                lhand_bbox = np.dot(
                    out["bb2img_trans"], lhand_bbox.transpose(1, 0)
                ).transpose(1, 0)[:, :2]
            ...
            if annot["rhand_bbox"] is not None:
                ...
                rhand_bbox = np.dot(
                    out["bb2img_trans"], rhand_bbox.transpose(1, 0)
                ).transpose(1, 0)[:, :2]
            ...
            if annot["face_bbox"] is not None:
                ...
                face_bbox = np.dot(
                    out["bb2img_trans"], face_bbox.transpose(1, 0)
                ).transpose(1, 0)[:, :2]
            ...
            if vis:
                ...
                param_save = {
                    "smplx_param": {
                        "root_pose": out["smplx_root_pose"].tolist(),
                        ...
                        "shape": out["smplx_shape"].tolist(),
                        "expr": out["smplx_expr"].tolist(),
